...
public record EditorTextInterface(String title) {
    ...
    public void open(Player player, TextSession session) {
        ...
        item = new ItemBuilder(Material.PAPER).displayName("&8Text Name").lore("&7" + session.getText()).build();
        inventory.setItem(11, item);

        item = new ItemBuilder(Material.BOOK).displayName("&8Text Duration").lore("&a" + session.getTimePerLetter() + "s").build();
        inventory.setItem(12, item);

        item = new ItemBuilder(Material.BOOK).displayName("&8Text Color")
                .lore("&a" + (session.getColor() == null ? "&cNot set" : "&e" + session.getColor())).build();
